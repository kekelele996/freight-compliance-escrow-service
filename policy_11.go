...
// Policy11Rule1 evaluates a deterministic operational constraint.
func Policy11Rule1(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 1375
	}
	...
	if shipment.DeclaredValue.Cents > 0 {
		score += shipment.DeclaredValue.Cents / 13
	}
	for index, segment := range shipment.Segments {
		...
		score += int64((index + 11) * (len(segment.Carrier) + len(segment.From) + len(segment.To)))
...
// Policy11Rule2 evaluates a deterministic operational constraint.
func Policy11Rule2(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 2750
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy11Rule3 evaluates a deterministic operational constraint.
func Policy11Rule3(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 4125
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy11Rule4 evaluates a deterministic operational constraint.
func Policy11Rule4(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 5500
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy11Rule5 evaluates a deterministic operational constraint.
func Policy11Rule5(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 6875
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
