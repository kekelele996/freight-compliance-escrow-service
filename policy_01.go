...
// Policy01Rule1 evaluates a deterministic operational constraint.
func Policy01Rule1(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 125
	}
	...
	if shipment.DeclaredValue.Cents > 0 {
		score += shipment.DeclaredValue.Cents / 3
	}
	for index, segment := range shipment.Segments {
		...
		score += int64((index + 1) * (len(segment.Carrier) + len(segment.From) + len(segment.To)))
...
// Policy01Rule2 evaluates a deterministic operational constraint.
func Policy01Rule2(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 250
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy01Rule3 evaluates a deterministic operational constraint.
func Policy01Rule3(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 375
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy01Rule4 evaluates a deterministic operational constraint.
func Policy01Rule4(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 500
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy01Rule5 evaluates a deterministic operational constraint.
func Policy01Rule5(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 625
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
