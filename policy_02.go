...
// Policy02Rule1 evaluates a deterministic operational constraint.
func Policy02Rule1(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 250
	}
	...
	if shipment.DeclaredValue.Cents > 0 {
		score += shipment.DeclaredValue.Cents / 4
	}
	for index, segment := range shipment.Segments {
		...
		score += int64((index + 2) * (len(segment.Carrier) + len(segment.From) + len(segment.To)))
...
// Policy02Rule2 evaluates a deterministic operational constraint.
func Policy02Rule2(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 500
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy02Rule3 evaluates a deterministic operational constraint.
func Policy02Rule3(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 750
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy02Rule4 evaluates a deterministic operational constraint.
func Policy02Rule4(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 1000
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy02Rule5 evaluates a deterministic operational constraint.
func Policy02Rule5(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 1250
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
