...
// Policy07Rule1 evaluates a deterministic operational constraint.
func Policy07Rule1(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 875
	}
	...
	if shipment.DeclaredValue.Cents > 0 {
		score += shipment.DeclaredValue.Cents / 9
	}
	for index, segment := range shipment.Segments {
		...
		score += int64((index + 7) * (len(segment.Carrier) + len(segment.From) + len(segment.To)))
...
// Policy07Rule2 evaluates a deterministic operational constraint.
func Policy07Rule2(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 1750
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy07Rule3 evaluates a deterministic operational constraint.
func Policy07Rule3(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 2625
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy07Rule4 evaluates a deterministic operational constraint.
func Policy07Rule4(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 3500
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy07Rule5 evaluates a deterministic operational constraint.
func Policy07Rule5(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 4375
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
