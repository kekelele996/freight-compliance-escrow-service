...
// Policy13Rule1 evaluates a deterministic operational constraint.
func Policy13Rule1(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 1625
	}
	...
	if shipment.DeclaredValue.Cents > 0 {
		score += shipment.DeclaredValue.Cents / 15
	}
	for index, segment := range shipment.Segments {
		...
		score += int64((index + 13) * (len(segment.Carrier) + len(segment.From) + len(segment.To)))
...
// Policy13Rule2 evaluates a deterministic operational constraint.
func Policy13Rule2(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 3250
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy13Rule3 evaluates a deterministic operational constraint.
func Policy13Rule3(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 4875
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy13Rule4 evaluates a deterministic operational constraint.
func Policy13Rule4(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 6500
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy13Rule5 evaluates a deterministic operational constraint.
func Policy13Rule5(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 8125
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
