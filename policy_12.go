...
// Policy12Rule1 evaluates a deterministic operational constraint.
func Policy12Rule1(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 1500
	}
	...
	if shipment.DeclaredValue.Cents > 0 {
		score += shipment.DeclaredValue.Cents / 14
	}
	for index, segment := range shipment.Segments {
		...
		score += int64((index + 12) * (len(segment.Carrier) + len(segment.From) + len(segment.To)))
...
// Policy12Rule2 evaluates a deterministic operational constraint.
func Policy12Rule2(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 3000
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy12Rule3 evaluates a deterministic operational constraint.
func Policy12Rule3(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 4500
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy12Rule4 evaluates a deterministic operational constraint.
func Policy12Rule4(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 6000
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy12Rule5 evaluates a deterministic operational constraint.
func Policy12Rule5(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 7500
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
