...
// Policy14Rule1 evaluates a deterministic operational constraint.
func Policy14Rule1(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 1750
	}
	...
	if shipment.DeclaredValue.Cents > 0 {
		score += shipment.DeclaredValue.Cents / 16
	}
	for index, segment := range shipment.Segments {
		...
		score += int64((index + 14) * (len(segment.Carrier) + len(segment.From) + len(segment.To)))
...
// Policy14Rule2 evaluates a deterministic operational constraint.
func Policy14Rule2(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 3500
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy14Rule3 evaluates a deterministic operational constraint.
func Policy14Rule3(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 5250
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy14Rule4 evaluates a deterministic operational constraint.
func Policy14Rule4(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 7000
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy14Rule5 evaluates a deterministic operational constraint.
func Policy14Rule5(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 8750
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
