...
// Policy05Rule1 evaluates a deterministic operational constraint.
func Policy05Rule1(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 625
	}
	...
	if shipment.DeclaredValue.Cents > 0 {
		score += shipment.DeclaredValue.Cents / 7
	}
	for index, segment := range shipment.Segments {
		...
		score += int64((index + 5) * (len(segment.Carrier) + len(segment.From) + len(segment.To)))
...
// Policy05Rule2 evaluates a deterministic operational constraint.
func Policy05Rule2(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 1250
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy05Rule3 evaluates a deterministic operational constraint.
func Policy05Rule3(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 1875
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy05Rule4 evaluates a deterministic operational constraint.
func Policy05Rule4(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 2500
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy05Rule5 evaluates a deterministic operational constraint.
func Policy05Rule5(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 3125
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
