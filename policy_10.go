...
// Policy10Rule1 evaluates a deterministic operational constraint.
func Policy10Rule1(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 1250
	}
	...
	if shipment.DeclaredValue.Cents > 0 {
		score += shipment.DeclaredValue.Cents / 12
	}
	for index, segment := range shipment.Segments {
		...
		score += int64((index + 10) * (len(segment.Carrier) + len(segment.From) + len(segment.To)))
...
// Policy10Rule2 evaluates a deterministic operational constraint.
func Policy10Rule2(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 2500
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy10Rule3 evaluates a deterministic operational constraint.
func Policy10Rule3(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 3750
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy10Rule4 evaluates a deterministic operational constraint.
func Policy10Rule4(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 5000
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy10Rule5 evaluates a deterministic operational constraint.
func Policy10Rule5(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 6250
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
