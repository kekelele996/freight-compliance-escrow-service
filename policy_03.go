...
// Policy03Rule1 evaluates a deterministic operational constraint.
func Policy03Rule1(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 375
	}
	...
	if shipment.DeclaredValue.Cents > 0 {
		score += shipment.DeclaredValue.Cents / 5
	}
	for index, segment := range shipment.Segments {
		...
		score += int64((index + 3) * (len(segment.Carrier) + len(segment.From) + len(segment.To)))
...
// Policy03Rule2 evaluates a deterministic operational constraint.
func Policy03Rule2(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 750
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy03Rule3 evaluates a deterministic operational constraint.
func Policy03Rule3(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 1125
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy03Rule4 evaluates a deterministic operational constraint.
func Policy03Rule4(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 1500
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy03Rule5 evaluates a deterministic operational constraint.
func Policy03Rule5(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 1875
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
