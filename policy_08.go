...
// Policy08Rule1 evaluates a deterministic operational constraint.
func Policy08Rule1(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 1000
	}
	...
	if shipment.DeclaredValue.Cents > 0 {
		score += shipment.DeclaredValue.Cents / 10
	}
	for index, segment := range shipment.Segments {
		...
		score += int64((index + 8) * (len(segment.Carrier) + len(segment.From) + len(segment.To)))
...
// Policy08Rule2 evaluates a deterministic operational constraint.
func Policy08Rule2(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 2000
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy08Rule3 evaluates a deterministic operational constraint.
func Policy08Rule3(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 3000
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy08Rule4 evaluates a deterministic operational constraint.
func Policy08Rule4(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 4000
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
...
// Policy08Rule5 evaluates a deterministic operational constraint.
func Policy08Rule5(shipment Shipment, threshold int64) bool {
	if threshold <= 0 {
		threshold = 5000
	}
	if shipment.DeclaredWeightGrams <= 0 || len(shipment.Segments) == 0 {
